...
const cloudAppleQuickBtn = document.getElementById('cloud-apple-quick-btn');
...
const watchAnalyticsSummary = document.getElementById('watch-analytics-summary');
const zoneRestEl = document.getElementById('zone-rest');
const zoneFatburnEl = document.getElementById('zone-fatburn');
...
function getBridgeBaseUrl() {
  return state.settings.watchBridgeUrl.replace('/api/watch/live', '');
}
...
async function cloudAppleQuickAuth() {
  if (!state.settings.watchBridgeUrl) {
    cloudStatus.textContent = 'Définissez d’abord URL bridge.';
    return;
  }
  const emailHint = cloudEmailInput.value.trim();
  const response = await fetch(`${getBridgeBaseUrl()}/api/auth/apple/demo`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ emailHint }),
  });
  const data = await response.json();
  if (!response.ok) throw new Error(data.error || 'apple_quick_auth_failed');
  state.settings.accountEmail = data.email;
  state.settings.sessionToken = data.token;
  cloudEmailInput.value = data.email;
  save();
  cloudStatus.textContent = `Connecté avec Apple: ${data.email}`;
}
...
function updateWatchAnalytics() {
  const points = state.watchLive.points;
  if (!points.length) {
    watchAnalyticsSummary.textContent = 'Analyse cardio en attente…';
    zoneRestEl.textContent = '0%';
    zoneFatburnEl.textContent = '0%';
    zoneCardioEl.textContent = '0%';
    zonePeakEl.textContent = '0%';
    recoveryScoreEl.textContent = '--';
    return;
  }
  const counts = { rest: 0, fatburn: 0, cardio: 0, peak: 0 };
  points.forEach((hr) => {
    if (hr < 100) counts.rest += 1;
    else if (hr < 130) counts.fatburn += 1;
    else if (hr < 160) counts.cardio += 1;
    else counts.peak += 1;
  });
  const total = points.length;
  zoneRestEl.textContent = `${Math.round((counts.rest / total) * 100)}%`;
  zoneFatburnEl.textContent = `${Math.round((counts.fatburn / total) * 100)}%`;
  zoneCardioEl.textContent = `${Math.round((counts.cardio / total) * 100)}%`;
  zonePeakEl.textContent = `${Math.round((counts.peak / total) * 100)}%`;

  const latest = points[points.length - 1];
  const avgRecent = Math.round(points.slice(-8).reduce((a, b) => a + b, 0) / Math.min(8, points.length));
...
cloudAppleQuickBtn.addEventListener('click', async () => {
  try { await cloudAppleQuickAuth(); } catch { cloudStatus.textContent = 'Échec connexion Apple rapide.'; }
});
